...
const GradeRegistrationView: React.FC<GradeRegistrationViewProps> = ({ 
  ...
  // --- GRADING HANDLERS ---
  const handleStatusChange = (studentId: string, period: string, status: 'GREEN' | 'RED') => {
  ...
  const handleNumericChange = (studentId: string, period: string, value: string) => {
      if (!data.allowedPeriods.includes(period)) return;
  ...
  const updateGrade = (studentId: string, period: string, value: string) => {
      setLocalData(prevData => {
          const newData = { ...prevData };
          const studentIndex = newData.studentsData.findIndex(s => s.id === studentId);
          if (studentIndex === -1) return prevData;
