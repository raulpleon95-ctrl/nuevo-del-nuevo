...
const DashboardView: React.FC<DashboardViewProps> = ({ data, onNavigate, currentUser, onUpdateData }) => {
  ...
  const activeStudentsCount = useMemo(() => {
      // Contar solo alumnos activos o sin estatus definido (legacy), excluir bajas y graduados
      return (data.studentsData || []).filter(s => s.status === 'active' || !s.status).length;
  }, [data.studentsData]);

  const activeTeachersCount = useMemo(() => {
      // Contar usuarios con rol de profesor
      return (data.users || []).filter(u => u.role === 'teacher').length;
  }, [data.users]);
