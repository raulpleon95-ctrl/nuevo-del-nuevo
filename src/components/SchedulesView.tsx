...
const SchedulesView: React.FC<SchedulesViewProps> = ({ data, onUpdateData, currentUser, currentCycle }) => {
  ...
  // 3. Personal Docente Completo (Para selector de Tutoría)
  const teachingStaff = useMemo(() => 
    [...academicStaff, ...technologyStaff].sort((a, b) => a.name.localeCompare(b.name)),
  [academicStaff, technologyStaff]);

  // 4. Personal de Apoyo (Red Escolar, Laboratorio, Apoyo)
  ...
  useEffect(() => {
      ...
      const layout = data.sabanaLayout as any;
      if (!layout || !layout.technology || !layout.tutoria) {
          const currentLayout = layout || { academic: [], support: [] };
          onUpdateData({
              ...data,
              sabanaLayout: {
                  academic: currentLayout.academic || [],
                  technology: currentLayout.technology || [],
                  tutoria: currentLayout.tutoria || [], // Ensure tutoria exists
  ...
  const sabanaLayout = data.sabanaLayout || { academic: [], technology: [], tutoria: [], support: [] };
  ...
  const handleCellChange = (teacherId: string, day: string, period: number, value: string, type: 'academic' | 'technology' | 'tutoria' | 'support') => {
    // Normalizar entrada (ej 1a -> 1A, 1at -> 1AT)
  ...
  const handleAddRow = (section: 'academic' | 'technology' | 'tutoria' | 'support') => {
  ...
  // Eliminar fila
  const handleRemoveRow = (section: 'academic' | 'technology' | 'tutoria' | 'support', index: number) => {
      const newLayout = { ...sabanaLayout };
  ...
  const handleSelectTeacherForRow = (section: 'academic' | 'technology' | 'tutoria' | 'support', index: number, teacherId: string) => {
      const newLayout = { ...sabanaLayout };
  ...
  const getTeacherInfo = (teacherId: string) => {
      ...
      // 1. Obtener disciplinas desde el Perfil (assignments)
      const disciplinesSet = new Set(teacher.assignments?.map(a => {
      ...
      // 2. DETECTAR TUTORÍA DESDE LA SÁBANA (SCHEDULES)
      // Si el profesor tiene asignaciones con type='tutoria', agregamos la materia automáticamente.
      const hasTutoria = (data.schedules || []).some(s => s.teacherId === teacherId && s.type === 'tutoria');
      if (hasTutoria) {
          disciplinesSet.add('TUTORÍA Y EDUCACIÓN SOCIOEMOCIONAL');
      }

      const disciplines = Array.from(disciplinesSet).join(' / ');
      
      // Extract unique groups from assignments
      const assignedGroups = teacher.assignments?.map(a => {
      ...
      // También extraer grupos de la sábana por si no están en asignaciones (ej: Tutoría solo en sábana)
      const scheduleGroups = (data.schedules || []).filter(s => s.teacherId === teacherId && s.type !== 'support').map(s => {
           return formatSectionCode(s.gradeGroup); // Puede devolver "1A" o "1° A,C"
      });

      // Unir y limpiar duplicados
      const groups = Array.from(new Set([...assignedGroups, ...scheduleGroups])).sort().join(', ');
  ...
  // --- LOGIC: GROUP VIEW (UPDATED FOR TUTORIA, INTEGRATION AND TEACHER DISPLAY) ---
  const getGroupScheduleMatrix = (gradeGroup: string) => {
      ...
      CLASS_PERIODS.forEach(p => {
          matrix[p] = {};
          WEEK_DAYS.forEach(d => {
              // Buscar entrada en la sábana
              // Coincidencia Académica: El grupo es exacto "1A" O es el grupo con sufijo T/I
              let entry = (data.schedules || []).find(s => {
                  const isAcademicMatch = (
                      s.gradeGroup === gradeGroup || 
                      s.gradeGroup === gradeGroup + 'T' ||
                      s.gradeGroup === gradeGroup + 'I'
                  ) && s.type === 'academic';
                  
                  const isTutoriaMatch = (
                      s.gradeGroup === gradeGroup || 
                      s.gradeGroup === gradeGroup + 'T'
                  ) && s.type === 'tutoria';
                  
                  const isTechMatch = (s.gradeGroup === techCode || s.gradeGroup === gradeGroup) && s.type === 'technology';
                  
                  return (isAcademicMatch || isTutoriaMatch || isTechMatch) && s.day === d && s.period === p;
              });
              
              if (entry) {
                  const teacher = (data.users || []).find(u => u.id === entry.teacherId);
                  
                  // Caso 1: TECNOLOGÍA (Varios profesores, se pone genérico)
                  if (entry.type === 'technology') {
                      matrix[p][d] = {
                          teacher: '', // No se pone profesor porque varía por alumno
                          subject: 'TECNOLOGÍA',
                          teacherId: ''
                      };
                  } 
                  // Caso 2: TUTORÍA (Sección Específica o Sufijo T)
                  else if (entry.type === 'tutoria' || entry.gradeGroup.endsWith('T')) {
                      matrix[p][d] = {
                          teacher: teacher ? teacher.name : '',
                          subject: 'TUTORÍA Y EDUCACIÓN SOCIOEMOCIONAL',
                          teacherId: teacher ? teacher.id : ''
                      };
                  }
                  // Caso 3: INTEGRACIÓN CURRICULAR (Sufijo I)
                  else if (entry.gradeGroup.endsWith('I')) {
                      matrix[p][d] = {
                          teacher: teacher ? teacher.name : '',
                          subject: 'INTEGRACIÓN CURRICULAR',
                          teacherId: teacher ? teacher.id : ''
                      };
                  }
                  // Caso 4: ACADÉMICO NORMAL
                  else if (teacher) {
  ...
  const getGroupSummary = (gradeGroup: string) => {
      ...
      (data.schedules || []).forEach(entry => {
          // Coincidencia: Grupo exacto "1A", "1AT" o "1AI"
          const isAcademicMatch = (
              entry.gradeGroup === gradeGroup || 
              entry.gradeGroup === gradeGroup + 'T' ||
              entry.gradeGroup === gradeGroup + 'I'
          ) && entry.type === 'academic';
          ...
          if (isAcademicMatch || isTutoriaMatch) {
              const teacher = (data.users || []).find(u => u.id === entry.teacherId);
              if (teacher) {
                  let subject = 'Asignatura';
                  
                  // Detección de claves especiales
                  if (entry.type === 'tutoria' || entry.gradeGroup.endsWith('T')) {
                      subject = 'TUTORÍA Y EDUCACIÓN SOCIOEMOCIONAL';
                  } else if (entry.gradeGroup.endsWith('I')) {
                      subject = 'INTEGRACIÓN CURRICULAR';
                  } else {
  ...
  const renderSabanaSection = (title: string, sectionKey: 'academic' | 'technology' | 'tutoria' | 'support', rows: string[], staffList: UserType[]) => (
      <>
        {/* Section Header Row */}
        <tr className="bg-slate-200 print:bg-slate-300 border-b border-slate-400">
            <td colSpan={2 + (WEEK_DAYS.length * CLASS_PERIODS.length)} className="px-2 py-1 font-bold text-xs uppercase sticky left-0 z-10 flex items-center justify-start gap-4 border-b border-slate-400 print:static">
                ...
                {sectionKey === 'academic' && <span className="text-[9px] font-normal lowercase bg-orange-100 px-2 rounded-full text-orange-800 print:bg-transparent">Sufijos: 'T' = Tutoría (1BT), 'I' = Integración (1BI)</span>}
            </td>
        </tr>
        ...
        {rows.map((teacherId, index) => (
            <tr key={`${sectionKey}-${index}`} className="hover:bg-slate-50 print:h-5">
                ...
                {/* Calculated Hours - STICKY WITH SHADOW TO SEPARATE FROM SCROLL */}
                <td className="px-1 py-1 text-center font-bold text-blue-600 sticky left-64 bg-slate-50 border-r border-b border-slate-300 z-10 print:text-black print:bg-white w-10 print:static print:border-black print:text-[8px] shadow-md print:shadow-none min-w-[40px]">
                    {teacherId ? calculateTotalHours(teacherId) : '-'}
                </td>
                {/* Input Cells */}
                {WEEK_DAYS.map(day => 
                    CLASS_PERIODS.map(period => (
                        <td key={`${day}-${period}`} className="p-0 border-r border-b border-slate-300 relative h-8 w-8 min-w-[32px] print:h-auto print:border-black print:w-auto">
                            <input 
                                type="text"
                                maxLength={15}
                                disabled={!teacherId}
                                className={`w-full h-full text-center uppercase focus:bg-blue-200 focus:outline-none font-bold bg-transparent print:text-black disabled:bg-slate-100 text-[10px] text-slate-900 print:text-[7px] ${sectionKey === 'technology' ? 'text-blue-900' : ''}`}
                ...
                <div className="flex-1 overflow-auto relative">
                    <div className="min-w-max print:min-w-0 print:w-full print:landscape">
                        ...
                        <table className="border-collapse border border-slate-300 w-full text-[10px] print:text-[7px] print:border-black">
                            <thead className="bg-slate-800 text-white sticky top-0 z-20 print:bg-slate-200 print:text-black print:static shadow-sm">
                                <tr>
                                    ...
                                    <th rowSpan={2} className="px-1 py-1 text-center uppercase tracking-wider w-10 border border-slate-600 bg-slate-800 sticky left-64 z-30 print:bg-slate-200 print:static print:border-black shadow-md print:shadow-none">
                                        HRS
                                    </th>
                                    {WEEK_DAYS.map(day => (
                            ...
                            <tbody className="bg-white divide-y divide-slate-300 print:divide-black">
                                {renderSabanaSection('CARGA ACADÉMICA', 'academic', sabanaLayout.academic, academicStaff)}
                                {renderSabanaSection('TUTORÍA Y EDUCACIÓN SOCIOEMOCIONAL', 'tutoria', sabanaLayout.tutoria || [], teachingStaff)}
                            ...
                            <tbody>
                                {(() => {
                                    ...
                                    return TIME_SLOTS.map((slot) => {
                                        ...
                                        const periodNum = slot.id as number;
                                        return (
                                            <tr key={periodNum} className="h-16">
                                                {/* Time Label */}
                                                ...
                                                {WEEK_DAYS.map(day => {
                                                    ...
                                                    return (
                                                        <td key={day} className="border border-black p-1 align-middle h-full">
                                                            {/* SOLO ASIGNATURA Y PROFESOR */}
                                                            {cell.subject && (
                                                                <div className="flex flex-col items-center justify-center h-full">
                                                                    <span className="font-bold text-[9px] uppercase leading-tight mb-1">{cell.subject}</span>
                                                                    {cell.teacher && <span className="text-[7px] uppercase text-slate-600 print:text-black leading-tight">{cell.teacher}</span>}
                                                                </div>
                                                            )}
                                                        </td>
                                                    );
