...
import StudentPortalView from './components/StudentPortalView'; // IMPORT NUEVO
...
export default function App() {
  ...
  const handleLogin = (loggedInUser: User, cycle: string) => {
    // Si es un usuario normal (staff), aseguramos permisos. Si es alumno, ya viene configurado.
    let userWithPerms = loggedInUser;
  ...
  // --- VIEW ROUTING ---
  if (!user) {
    return <LoginView onLogin={handleLogin} users={schoolData.users || []} studentsData={schoolData.studentsData || []} isLoading={isInitialLoading} />;
  }

  // ALUMNO VIEW
  if (user.role === 'student') {
      return <StudentPortalView studentUser={user} data={schoolData} onLogout={handleLogout} currentCycle={currentCycle} />;
  }
        ...
        <main className="flex-1 overflow-y-auto scroll-smooth">
            <header className="bg-white/80 backdrop-blur-sm border-b border-slate-200 p-4 sticky top-0 z-20 hidden md:flex justify-between items-center print:hidden">
                <div className="flex items-center text-sm text-slate-500">
                   <span className="font-medium text-slate-800 mr-2">{schoolData.name} <span className="text-emerald-600 font-bold bg-emerald-50 px-2 rounded">v5.3</span></span>
