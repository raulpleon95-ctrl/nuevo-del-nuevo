...
// Función de inicialización robusta (Modular SDK v9+)
const initFirebase = () => {
    ...
    if (configToUse) {
        try {
            ...
            if (getApps().length === 0) {
                ...
                // CONFIGURACIÓN MODERNA DE PERSISTENCIA (v11+)
                // Esto reemplaza a enableIndexedDbPersistence y maneja mejor múltiples pestañas
                try {
                    ...
                    console.log(`🔥 Firebase conectado exitosamente (Persistencia Moderna Activa).`);
                } catch (initErr) {
                    console.warn("Fallo al inicializar persistencia moderna, usando configuración por defecto:", initErr);
                    db = getFirestore(app);
                }

            } else {
                app = getApp();
                // Si la app ya existe, obtenemos la instancia de Firestore existente
                db = getFirestore(app);
            }
...
export const subscribeToData = (callback: (data: SchoolData) => void, onError?: (error: any) => void) => {
    ...
    const unsub = onSnapshot(doc(db, "schools", "default"), (docSnap) => {
        if (docSnap.exists()) {
            ...
            // Si el documento no existe (primera vez), inicializamos con null o manejamos en la UI
            // callback(null); 
        }
    }, (error) => {
